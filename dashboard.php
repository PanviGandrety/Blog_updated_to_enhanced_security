...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'create') {
    if (!csrf_check($_POST['csrf'] ?? '')) $errors[] = 'Invalid request.';
        ...
        if (empty($errors)) {
            ...
            if ($stmt->execute()) $success = 'Post created.';
...
if ($search !== '') {
    ...
    $stmt = $conn->prepare("SELECT p.id, p.user_id, p.title, p.content, p.created_at, u.username FROM posts p JOIN users u ON p.user_id = u.id WHERE p.title LIKE ? OR p.content LIKE ? ORDER BY p.created_at DESC LIMIT ? OFFSET ?");
    ...
    $stmt = $conn->prepare("SELECT p.id, p.user_id, p.title, p.content, p.created_at, u.username FROM posts p JOIN users u ON p.user_id = u.id ORDER BY p.created_at DESC LIMIT ? OFFSET ?");
      ...
      <div class="card-body">
        ...
        <div>
          <?php if ($role === 'admin' || $row['user_id'] == $user_id): ?>
            <a href="edit_post.php?id=<?php echo $row['id']; ?>" class="btn btn-sm btn-outline-primary me-1">Edit</a>
